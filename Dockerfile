...
RUN apk update
RUN apk upgrade
RUN apk add --no-cache bash libpng-dev libxml2-dev icu-dev tidyhtml-dev libsodium-dev
RUN docker-php-ext-install -j2 gd soap intl sockets calendar exif pdo_mysql tidy zip sodium pcntl
...
RUN apk add --no-cache bash libpng libxml2 icu tidyhtml libsodium libmemcached gettext imagemagick
